...
describe('remapSeedWorkflowIds', () => {
	...
	it('uniquifies the workflow NAME too, and follows it through the messages', () => {
		// TRUST-273: a leftover copy sharing the name is a candidate the agent can
		// ground on instead. The seeded history has to move with the rename, or the
		// agent's own record of what it built stops matching the instance.
		const seed = makeSeed();
		seed.messages.push({
			id: 'm-name',
			type: 'llm',
